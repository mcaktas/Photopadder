...
def process_image(
    ...
    # Step 1 — pad to aspect ratio
    canvas_w, canvas_h = compute_canvas_sizes_no_resize(img.size, ratio_short, ratio_long)
...
class PadApp:
    def __init__(self, root):
        ...
        # UI --- Input folder
        tk.Label(root, text="Input folder:").grid(row=0, column=0, sticky="w", padx=5)
        tk.Entry(root, textvariable=self.input_dir_var, width=50).grid(row=0, column=1, padx=5)
        tk.Button(root, text="Browse", command=self.browse_input).grid(row=0, column=2, padx=5)

        # Output folder
        tk.Label(root, text="Output folder:").grid(row=1, column=0, sticky="w", padx=5)
        tk.Entry(root, textvariable=self.output_dir_var, width=50).grid(row=1, column=1, padx=5)
        tk.Button(root, text="Browse", command=self.browse_output).grid(row=1, column=2, padx=5)

        # Select preset ratio
        tk.Label(root, text="Aspect ratio (short:long):").grid(row=2, column=0, sticky="w", padx=5)
        tk.OptionMenu(root, self.ratio_label_var, *self.ratio_options.keys()).grid(row=2, column=1, sticky="w", padx=5)

        # Custom ratio
        tk.Label(root, text="Custom ratio (e.g. 3:7):").grid(row=3, column=0, sticky="w", padx=5)
        tk.Entry(root, textvariable=self.custom_ratio_var, width=10).grid(row=3, column=1, sticky="w", padx=5)

        # Extra border
        tk.Label(root, text="Extra border (%):").grid(row=4, column=0, sticky="w", padx=5)
        tk.Entry(root, textvariable=self.border_var, width=10).grid(row=4, column=1, sticky="w", padx=5)

        # Border color picker
        tk.Label(root, text="Padding color:").grid(row=5, column=0, sticky="w", padx=5)
        self.color_label = tk.Label(root, text=self.border_color_hex, bg=self.border_color_hex, width=10)
        self.color_label.grid(row=5, column=1, sticky="w", padx=5)
        tk.Button(root, text="Pick", command=self.pick_color).grid(row=5, column=2, padx=5)
